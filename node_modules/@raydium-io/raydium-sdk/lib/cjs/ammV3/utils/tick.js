...
exports.TICK_ARRAY_BITMAP_SIZE = 1024;
class TickUtils {
    ...
    static getTickArrayStartIndexByTick(tickIndex, tickSpacing) {
        let startIndex = tickIndex / (exports.TICK_ARRAY_SIZE * tickSpacing);
        if (tickIndex < 0 && tickIndex % (exports.TICK_ARRAY_SIZE * tickSpacing) != 0) {
            startIndex = Math.ceil(startIndex) - 1;
        }
        else {
            startIndex = Math.floor(startIndex);
        }
        return startIndex * (tickSpacing * exports.TICK_ARRAY_SIZE);
    }
    ...
    static getNextTickArrayStartIndex(lastTickArrayStartIndex, tickSpacing, zeroForOne) {
        return zeroForOne ? lastTickArrayStartIndex - tickSpacing * exports.TICK_ARRAY_SIZE : lastTickArrayStartIndex + tickSpacing * exports.TICK_ARRAY_SIZE;
    }
    ...
    static getInitializedTickArrayInRange(tickArrayBitmap, tickSpacing, tickArrayStartIndex, expectedCount) {
        if (tickArrayStartIndex % (tickSpacing * exports.TICK_ARRAY_SIZE) != 0) {
            throw new Error('Invild tickArrayStartIndex');
        }
        const tickArrayOffset = Math.floor(tickArrayStartIndex / (tickSpacing * exports.TICK_ARRAY_SIZE)) + 512;
        return [
            ...
            ...TickUtils.searchHightBitFromStart(tickArrayBitmap, tickArrayOffset, exports.TICK_ARRAY_BITMAP_SIZE, expectedCount, tickSpacing)
        ];
    }
    static getAllInitializedTickArrayStartIndex(tickArrayBitmap, tickSpacing) {
        // find from offset 0 to 1024
        return TickUtils.searchHightBitFromStart(tickArrayBitmap, 0, exports.TICK_ARRAY_BITMAP_SIZE, exports.TICK_ARRAY_BITMAP_SIZE, tickSpacing);
    }
    ...
    static searchLowBitFromStart(tickArrayBitmap, start, end, expectedCount, tickSpacing) {
        ...
        for (let i = start; i >= end; i--) {
            if (tickArrayBitmap.shrn(i).and(new bn_js_1.default(1)).eqn(1)) {
                const nextStartIndex = (i - 512) * (tickSpacing * exports.TICK_ARRAY_SIZE);
